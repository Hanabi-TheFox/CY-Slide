...
public class AStarAlgo {
    ...
    public void aStar(Tile[][] currentTile, Tile[][] finalTile) {
        ...
        // Add the initial state to the open set with g = 0 and f = heuristic
        int initialH = calculeMisplacedTileCount(currentTile, finalTile);
        State initialState = new State(currentTile, 0, initialH);
        openList.add(initialState);
        ...
        while (!openList.isEmpty() ) {
            // Obtenir l'état actuel de la file openList
            State current = openList.poll();
            currentTile = cloneTiles(current.tiles);

            
            // Display information before each passage
            // System.out.println("Passage " + passageCount);
            System.out.println("Nombre de case mal placées: " + current.f);
            System.out.println("Tiles:");
            //printState(current.tiles);
            System.out.println("g: " + current.g);
            System.out.println("f: " + current.f);
            ...
            if (neighbors.isEmpty()) {
                ...
                for (Tile[][] neighbor : neighbors) {
                    // Calculer les coûts pour l'état voisin
                    int neighborG = current.g + 1;
                    int neighborH = calculeMisplacedTileCount(currentTile, finalTile);
                    int neighborF = neighborG + neighborH;
    ...
    private List<Tile[][]> generateNeighbors(Tile[][] currentTile, Tile[][] finalTile) {
        // Trouvez les coordonnées de la case vide
        int emptyRow = -1;
        int emptyCol = -1;
        EmptyTile emptyTile = new EmptyTile(emptyRow, emptyCol);
        for (int i = 0; i < currentTile.length; i++) {
            for (int j = 0; j < currentTile[i].length; j++) {
                if (currentTile[i][j].getType() == -1) {
                    emptyRow = i;
        ...
        // Générez le voisin en permutant les cases adjacentes avec la case vide
        List<Tile[][]> neighbors = new ArrayList<>();
        String[] directions = {"UP", "DOWN", "RIGHT", "LEFT"}; // Haut, bas, droite, gauche
        ...
        for (String direction : directions) {
            ...
            if (emptyTile.mouvementAvailable2(direction, currentTile)) {
                Tile[][] neighbor = cloneTiles(currentTile);
                emptyTile.SwapTile(emptyRow, emptyCol, newRow, newCol, neighbor);
    ...
    public boolean isFinalState(Tile[][] currentTile, Tile[][] finalTile) {
        ...
        for (int i = 0; i < currentTile.length; i++) {
            for (int j = 0; j < currentTile[i].length; j++) {
                Tile current = currentTile[i][j];
                Tile expected = finalTile[i][j];
    
                // Ignore empty tiles
                if (current instanceof EmptyTile || expected instanceof EmptyTile) {
                    continue;
                }
    
                if (current instanceof NumberTile && expected instanceof NumberTile) {
                    NumberTile currentNumberTile = (NumberTile) current;
                    NumberTile finalNumberTile = (NumberTile) expected;
                    if (currentNumberTile.getNumber() != finalNumberTile.getNumber()) {
                        return false;
                    }
                } else {
                    // Tiles are of different types
                    return false;
                }
            }
        }
    ...
    public int calculeMisplacedTileCount(Tile[][] currentTile, Tile[][] finalTile) {
        int count = 0;
        for (int i = 0; i < currentTile.length; i++) {
            for (int j = 0; j < currentTile[i].length; j++) {
